...
class QuizQuestion(SQLModel, table=True):
    """测验问题表，存储测验中的问题"""
    id: Optional[int] = Field(default=None, primary_key=True)
    quiz_id: int = Field(foreign_key="quiz.id", description="关联测验ID")
...
class QuizResult(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    quiz_attempt_id: int = Field(foreign_key="quizattempt.id", description="关联测验尝试ID")
    question_id: int = Field(foreign_key="quizquestion.id", description="关联问题ID")
